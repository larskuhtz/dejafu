...
--
-- @since unreleased
autocheck :: (MonadConc m, MonadIO m, Eq a, Show a)
  => DejaFu m a
...
--
-- @since unreleased
autocheckWay :: (MonadConc m, MonadIO m, Eq a, Show a)
  => Way
...
--
-- @since unreleased
dejafu :: (MonadConc m, MonadIO m, Show a)
  => DejaFu m a
  -- ^ The computation to test
  -> (String, Predicate a)
...
--
-- @since unreleased
dejafuWay :: (MonadConc m, MonadIO m, Show a)
  => Way
  -- ^ How to run the concurrent program.
  -> MemType
  -- ^ The memory model to use for non-synchronised @CRef@ operations.
...
--
-- @since unreleased
dejafuDiscard :: (MonadConc m, MonadIO m, Show a)
...
--
-- @since unreleased
dejafus :: (MonadConc m, MonadIO m, Show a)
  => DejaFu m a
  -- ^ The computation to test
  -> [(String, Predicate a)]
...
--
-- @since unreleased
dejafusWay :: (MonadConc m, MonadIO m, Show a)
  => Way
  -- ^ How to run the concurrent program.
  -> MemType
  -- ^ The memory model to use for non-synchronised @CRef@ operations.
