...
litmusTest :: (Eq a, Show a) => String -> DejaFu IO a -> [a] -> [a] -> [a] -> Test
...
compareTest act = do
  putStr "DejaFu-SQ:  " >> results SequentialConsistency >>= putStrLn
  putStr "DejaFu-TSO: " >> results TotalStoreOrder >>= putStrLn
  putStr "DejaFu-PSO: " >> results PartialStoreOrder >>= putStrLn
