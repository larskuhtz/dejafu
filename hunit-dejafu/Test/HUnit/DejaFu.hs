...
module Test.HUnit.DejaFu
  ...
  -- for @DejaFu IO@. These instances try all executions, reporting as
  -- failures the cases which throw an 'HUnitFailure' exception.
  --
  -- @instance Testable   (DejaFu IO ())@
  -- @instance Assertable (DejaFu IO ())@
  --
...
-- | @since unreleased
instance Testable (Conc.DejaFu IO ()) where
  test conc = TestCase (assert conc)

-- | @since unreleased
instance Assertable (Conc.DejaFu IO ()) where
  assert conc = do
